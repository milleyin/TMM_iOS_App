...
- (void)setShowTip
{
    if (self.annotation.subtitle) {
        ...
        {
            self.calloutView = [[BusAnnotationView alloc] initWithFrame:CGRectMake(0, 0, 200, 40)];
        ...
        CGRect m_rect = CGRectMake(rect.origin.x-(rect.size.width)/2+8, rect.origin.y-(rect.size.height+20)+2, rect.size.width+16, rect.size.height+20);
        
        [self.calloutView setFrame:m_rect];
        self.calloutView.titleLabel.text = self.annotation.subtitle;
        [self.calloutView.titleLabel setFrame:CGRectMake(8, 6, rect.size.width, rect.size.height)];
