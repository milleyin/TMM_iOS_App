...
@synthesize viewShowMode = _viewShowMode;
...
- (void)initProperties
{
    _sections = [[NSArray alloc]initWithObjects:@"偏好设置", nil];
    
    _options = [[NSArray alloc]initWithObjects:@[@"视图模式", @"昼夜模式"], nil];
}
...
- (void)initTableView
{
    _optionTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.bounds) - kFinishButtonHeight - kOptionTableViewHieght, CGRectGetWidth(self.bounds), kOptionTableViewHieght) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([optionName isEqualToString:@"视图模式"])
    {
...
- (UITableViewCell *)tableViewCellForViewMode
{
    ...
    optionNameLabel.text = @"视图模式:";
    
    if (self.viewShowMode == AMapNaviViewShowModeMapNorthDirection)
    {
        [_viewModeSeg setSelectedSegmentIndex:0];
    }
    else if (self.viewShowMode == AMapNaviViewShowModeCarNorthDirection)
    {
...
- (void)viewModeSegmentedControlAction:(id)sender
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(moreMenuViewViewModeChangeTo:)])
    {
        [self.delegate moreMenuViewViewModeChangeTo:(AMapNaviViewShowMode)selectedIndex];
    }
}
